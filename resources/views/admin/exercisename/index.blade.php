...
@section('title',__('All ExerciseName'))
@section('maincontent')
<!-- Start Breadcrumbbar -->                    
...
<div class="col-md-12 col-lg-6 text-right">
  <div class="top-btn-block">
    <a href="{{route('exercisename.create')}}" class=" btn btn-primary-rgba mr-2"><i
        class="feather icon-plus mr-2"></i>{{ __("Add Exercisename") }}</a>
    ...
    <a href="{{ route('exn.index') }}" class="btn btn-primary-rgba mr-2"><i
    ...
    <div class="card m-b-30">
      <div class="card-header">
        <div class="row align-items-center">
          <div class="col-6">
            <h5 class="card-title mb-0">{{ __("All ExerciseName") }}</h5>
          </div>
      ...
      <div class="card-body">
        <div class="table-responsive">
          <table id="datatable-buttons" class="table table-border">
            ...
            <tbody>
              @foreach($exercisename as $key => $list)
              <tr>
                 <td>
                  <div class='inline'>
                    ...
                    <div class="modal-dialog modal-sm">
                      ...
                      <div class="modal-content">
                        ...
                        <div class="modal-footer">
                          <form id="bulk_delete_form" method="post" action="{{ route('exercisename.bulk_delete') }}">
                            @csrf
                            @method('POST')
                ...
                <td>{{$list->exercisename}}</td>
                <td>{{$list->body_part}}</td>
                <td> @if($list->type)
                  @foreach($list->type as $key => $t)
                  @php
                  $type = App\Type::find($t);
                  @endphp
                  @if(isset($type))
                  <span class="badge badge-primary">{{ ucfirst($type->name) }}</span>
                  @endif
                  @endforeach
                  @endif</td>
                ...
                Admin' )
                <td>
                  {!! Form::open(['method' => 'PUT', 'route' => ['exercisename.status', $list->id]]) !!}
                ...
                <td>
                  <div class="button-list">
                    <a href="{{route('exercisename.edit', $list->id)}}" class="btn btn-xm btn-success-rgba"><i
                    ...
                    <div id="deleteModal{{$list->id}}" class="delete-modal modal fade" role="dialog">
                      <div class="modal-dialog modal-sm">
                        ...
                        <div class="modal-content">
                          ...
                          <div class="modal-footer">
                            {!! Form::open(['method' => 'DELETE', 'route' => ['exercisename.destroy', $list->id]]) !!}
                            <button type="reset" class="btn btn-primary-rgba" data-dismiss="modal">{{ __("No") }}</button>
