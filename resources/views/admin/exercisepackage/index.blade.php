...
<div class="col-md-12 col-lg-6 text-right">
  <div class="top-btn-block">
    <a href="{{route('exercisepackage.create',['id' => app('request')->input('id')])}}"
    class="btn btn-primary-rgba mr-2"><i class="feather icon-plus mr-2"></i>{{ __("Add Exercise") }}</a>
      ...
      <div class="card-body">
        <div class="table-responsive">
          <table id="datatable-buttons" class="table table-border">
            ...
            @if (isset($exercisepackages))
            <tbody>
              @foreach($exercisepackages as $key => $list)
              <tr>
                <td>
                  <div class='inline'>
                    ...
                    <div class="modal-dialog modal-sm">
                      ...
                      <div class="modal-content">
                        ...
                        <div class="modal-footer">
                          <form id="bulk_delete_form" method="post" action="{{ route('exercisepackage.bulk_delete') }}">
                            @csrf
                            @method('POST')
                ...
                <td>
                  @if($list->exercise_package)
                  @foreach($list->exercise_package as $key => $d)
                  @php
                   $package_u = App\Exercise::where('id', $d)->first();
                   @endphp
                    @if(isset($package_u))
                  <span class="badge badge-primary">{{ ucfirst($package_u->exercise_package) }}</span>
                  @endif
                  @endforeach
                  @endif
                </td>
                 <td>
                  {{ $list->exercise->detail }}
                </td>
                <td>
                  @foreach($list->exercise->session_id as $value)
                  @php
                ...
                <td>
                  @foreach($list->exercise->day as $value)
                  @php
                ...
                Admin' )
                <td>
                  {!! Form::open(['method' => 'PUT', 'route' => ['exercisepackage.status', $list->id]]) !!}
                ...
                <td>
                  <div class="button-list">
                    <a href="{{route('exercisepackage.edit', $list->id)}}" class="btn btn-xm btn-success-rgba"><i
