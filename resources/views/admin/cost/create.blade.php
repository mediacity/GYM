        ...
        <div class="card m-b-30">
            <div class="card-header">
                <div class="row align-items-center">
                    <div class="col-12">
                        <h5 class="card-title mb-0">{{ __("Add Cost") }}</h5>
                    </div>
                </div>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-md-12">
                        {!! Form::open(['method' => 'POST', 'route' => 'cost.store','files' => true , 'class' =>
                        'form-light form' , 'novalidate']) !!}
                        <div class="admin-form">
                            {{ $errors->has('Cost') ? ' has-error' : '' }}
                            {!! Form::label('cost', 'Cost',['class'=>'required']) !!}<span class="text-danger">*</span>
                            {!! Form::text('cost', null, ['class' => 'form-control', 'required','placeholder' =>
                            'Please Enter Cost']) !!}
                            <small class="text-danger">{{ $errors->first('cost') }}</small>
                            <small class="text-muted"> <i class="text-dark feather icon-help-circle"></i>{{ __(" Enter Cost eg: 10,000 , 12,000") }}</small>
                        </div>
                        <br>
