...
@section('title',__('All PT Subscription'))
@section('maincontent')
<!-- Start Breadcrumbbar -->
...
@endslot
@slot('menu1')
{{ __('All PT Subscription') }}
@endslot
@slot('button')
<div class="col-md-6 col-lg-6">
    @if(auth()->user()->can('pt.add'))
    <a href="{{route('ptsubscription.create')}}" class="float-right btn btn-primary-rgba mr-2"><i
            class="feather icon-plus mr-2"></i>{{ __("Add PT Subscription") }}</a>
...
<div class="card m-b-30">
    <div class="card-header">
        <div class="row align-items-center">
            <div class="col-6">
                <h5 class="card-title mb-0">{{ __("All PT Subscription") }}</h5>
            </div>
        </div>
    </div>
    <div class="card-body">
            ...
            <table id="datatable-buttons" class="table table-border">
                <thead>
                    <tr>
                        ...
                        <th>{{ __("Goal") }}</th>
                        <th>{{ __("Issue") }}</th>
                        @if(auth()->user()->can('pt.view'))
                        <th>{{ __("Actions") }}</th>
                        @endif
                    </tr>
                </thead>
                @if (isset($ptsubscription))
                <tbody>
                    @foreach($ptsubscription as $key => $list)
                    <tr>
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            <form id="bulk_delete_form" method="post"
                                                action="{{ route('ptsubscription.bulk_delete') }}">
                                                @csrf
                                                @method('POST')
                        ...
                        <td>
                            ...
                            <div class="button-list">
                                ...
                                <a href="{{route('ptsubscription.edit', $list->id)}}"
                                    ...
                                    <div class="modal-dialog modal-sm">
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                ['ptsubscription.destroy', $list->id]]) !!}
